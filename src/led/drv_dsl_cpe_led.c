...
static DSL_int_t DSL_DRV_LED_Flash ( DSL_void_t )
{
   ...
   DSL_boolean_t bWakeUp = DSL_FALSE;

   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: IN - DSL_DRV_LED_Flash"DSL_DRV_CRLF,
   ...
   if (!stop_led_module)
   {
      if (pContext->bLedNeedToFlash == DSL_FALSE)
      {
         /* asking to flash led */
   ...
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: OUT - DSL_DRV_LED_Flash"DSL_DRV_CRLF,
      DSL_DEV_NUM(pContext)));
   ...
   DSL_DRV_ThreadParams_t *param)
{
   ...
   while (!stop_led_module)
   {
      ...
      if( nErrCode != DSL_SUCCESS )
      {
         DSL_DEBUG(DSL_DBG_ERR,
            (pContext, SYS_DBG_ERR"DSL[%02d]: ERROR - [%s %d]: Failed to get "
      ...
      if (pContext->bLedNeedToFlash && (pContext->bLedStatusOn == DSL_FALSE))
      {
         ...
         {
            DSL_DEBUG(DSL_DBG_ERR,
               (pContext, SYS_DBG_ERR"[%s %d]: CMV Write fail!" DSL_DRV_CRLF, __func__, __LINE__));
         }
   ...
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: OUT - DSL_DRV_LED_Poll thread"DSL_DRV_CRLF,
      DSL_DEV_NUM(pContext)));
   ...
   DSL_Context_t *pContext)
{
   ...
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: IN - DSL_DRV_LED_ModuleInit"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));

   if (pContext->bLedInit)
   {
      DSL_DEBUG(DSL_DBG_WRN,
         (pContext, SYS_DBG_WRN"DSL[%02d]: WARNING - LED module already started"
         DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   if (pVer->bValid == DSL_TRUE)
   {
      ...
      {
         DSL_DEBUG(DSL_DBG_ERR,
            (pContext, SYS_DBG_ERR"DSL[%02d]: ADSL Data LED handling included but not implemented for Danube!"
            DSL_DRV_CRLF,
            DSL_DEV_NUM(pContext)));
   ...
   else
   {
      DSL_DEBUG(DSL_DBG_ERR, (pContext,
         SYS_DBG_ERR"DSL[%02d]: ERROR - No firmware version available"
         DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
...
#error Please use --with-adsl-data-led-flash-frequency=[1-32] to define ADSL Data Flash Frequency
#endif
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: ADSL data led flashing at %d Hz" DSL_DRV_CRLF,
   ...
   {
      DSL_DEBUG(DSL_DBG_ERR,
         (pContext, SYS_DBG_ERR"DSL[%02d]: ADSL DATA LED Simulator task start failed!"
         DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   DSL_Context_t *pContext)
{
   DSL_Error_t nErrCode = DSL_SUCCESS;
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: IN - DSL_DRV_LED_FirmwareInit"
      DSL_DRV_CRLF, DSL_DEV_NUM(pContext)));
   ...
   */
   ...
   DSL_DEBUG(DSL_DBG_MSG,
      (pContext, SYS_DBG_MSG"DSL[%02d]: OUT - DSL_DRV_LED_FirmwareInit, retCode=%d"
...
#endif /* #ifdef INCLUDE_DEPRECATED*/
